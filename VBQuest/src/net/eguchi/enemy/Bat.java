...
public class Bat extends EnemyBase{


	public Bat(MainView v, double x, double y, Map map) {
		super(v, x, y, map);
		this.map = map;
		this.iX = x;
		this.iY = y;
		...
		iWidth = v.getImgWidth(Img.bat);
		iHeight =  v.getImgHeight(Img.bat);
	...
	public void onBitmap(int offsetX, int offsetY){
		...
		//敵と衝突したとき、プレイヤーが点滅
		if(invtime %2 == 0){v.drawBitmap(Img.bat, src, dst);}
