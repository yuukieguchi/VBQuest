...
public class Devil extends EnemyBase{
			
	public Devil(MainView v ,double x, double y ,Map map) {
		// 親クラスのコンストラクタを実行する
		super( v, x, y, map );
		this.v= v;
		this.map = map;
		...
		//[ 2 ] 画像サイズ取得
		iWidth 	= v.getImgWidth(Img.devil);
		iHeight = v.getImgHeight(Img.devil);
	...
	public void onBitmap(int offsetX, int offsetY){
		...
		//アニメーション
		if(invtime%2 == 0){v.drawBitmap(Img.devil, src, dst);}
